export async function loadFonts () {
  ...
  let apiurl = 'https://forms.gle/XPrTX3qpxbCrNcQR7'
...
function add() {
	func(apiurl, number, "1").then(data => alert("Кормушка пополнена")).catch(function(error) {
...
function empted() {
	func(apiurl, number, "0").then(data => alert("Высылаем волонтера")).catch(function(error) {
		console.log('request failed', error)
	})
}
